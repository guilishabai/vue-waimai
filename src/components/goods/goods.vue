<template>
  <div class="goods">
      ...
      <ul>
        <li v-for="(item, index) in goods" :key="item.id" class="menu-item" :class="{'current': currentIndex === index}" @click="selectMenu(index, $event)" ref="menuItem">
...
export default {
  ...
  methods: {
    _initScroll() {
      this.meunScroll = new BetterScroll(this.$refs.menuWrapper, {
        click: true
      })
